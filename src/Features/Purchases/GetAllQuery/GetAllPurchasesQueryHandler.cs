...
public sealed record GetAllPurchasesQueryHandler(IPurchaseRepository purchaseRepository) : IRequestHandler<GetAllPurchasesQuery, PagedResponse<Purchase>>
{
    private readonly IPurchaseRepository _purchaseRepository = purchaseRepository;
    public async Task<PagedResponse<Purchase>> Handle(GetAllPurchasesQuery request, CancellationToken cancellationToken)
    {
        var purchases = await _purchaseRepository.GetAllAsync(request.PageNumber, request.PageSize, cancellationToken);

        return new PagedResponse<Purchase>(purchases);
    }
}